...
lr = 0.0006  # 学习率
#——————————————————导入数据——————————————————————
f = open(r'c:\temp\stock_price.csv')
df = pd.read_csv(f)  # 读入股票数据
data = df.iloc[:, 2:10].values  # 取第3-10列

checkpoint_dir = 'c:\\Temp\\checkpoint\\'
        ...
        for i in range(50):
            ...
            if i % 200 == 0:
                print("保存模型：", saver.save(
                    sess, checkpoint_dir + 'stock2.model', global_step=i))
...
def prediction(time_step=20):
    ...
    with tf.Session() as sess:
        ...
        acc = np.average(np.abs(
            test_predict - test_y[:len(test_predict)]) / test_y[:len(test_predict)])  # 偏差
        #以折线图表示结果
        plt.figure()
