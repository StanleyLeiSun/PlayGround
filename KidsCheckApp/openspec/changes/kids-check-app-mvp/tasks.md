...
- [x] 1.1 Initialize FastAPI project structure (main.py, routers/, models/, schemas/, services/, config.py)
- [x] 1.2 Configure PostgreSQL connection with SQLAlchemy async (database.py, alembic for migrations)
- [x] 1.3 Create Alembic migration for tables: user, child, task_template, conditional_task, daily_task, check_in_photo, point_account, point_transaction, reward, reward_redemption
- [x] 1.4 Create seed data migration: 6 users (爸爸/妈妈 role=parent, 爷爷/奶奶/姥姥/姥爷 role=grandparent, password=123456 hashed), 2 children (萝卜 age=8, 蚕豆 age=5), 2 point accounts (balance=0)
- [x] 1.5 Setup JWT authentication middleware (auth.py: token generation, validation, role extraction)
- [x] 1.6 Setup file upload directory structure and static file serving for photos
- [x] 1.7 Initialize Android project with Kotlin + Jetpack Compose, configure Retrofit HTTP client
...
- [x] 3.1 Implement GET /api/templates/{child_id} (return templates grouped by weekday)
- [x] 3.2 Implement POST /api/templates/{child_id} (create template with weekday, title, type, points)
...
- [x] 4.1 Implement daily task generation service (query templates by weekday, create daily_task records)
- [x] 4.2 Configure APScheduler to run generation at 00:00 UTC+8 daily
...
- [x] 5.1 Implement POST /api/daily-tasks/{id}/check-in (validate task status, handle photo requirement by type)
- [x] 5.2 Implement photo upload handling (compress validation, UUID naming, store to filesystem)
- [x] 5.3 Implement check_in_photo record creation with photo_url, uploaded_by, uploaded_at
- [x] 5.4 Implement duplicate check-in prevention (409 if already done)
...
- [x] 7.1 Implement point awarding service (add to balance, create transaction, idempotency via task reference)
- [x] 7.2 Implement GET /api/points/{child_id} (balance + paginated transaction history)
...
- [x] 8.1 Create action_log table migration (id, user_id, action, target_type, target_id, metadata JSON, created_at)
...
- [x] 9.1 Build login screen (user list selector + password input, token storage)
- [x] 9.2 Build bottom navigation (3 tabs: 今日任务, 进度, 我的)
- [x] 9.3 Build today's task list screen (child switch tabs: 萝卜/蚕豆, task cards, pending/done states)
- [x] 9.4 Build check-in bottom sheet (photo capture for written, confirm-only for reading, celebration animation)
- [x] 9.5 Build progress screen (date picker, progress bar, timeline, photo thumbnails)
- [x] 9.6 Build "我的" screen (child points, menu items with role-based visibility)
- [x] 9.7 Build task template management screen (weekday groups, conditional tasks section, parent-only)
- [x] 9.8 Build voice input integration (Android STT → API call → confirmation card)
- [x] 9.9 Build rewards screen (reward list, redeem button, point balance display)
- [x] 9.10 Implement offline caching for today's task list (Room database)
- [x] 9.11 Implement photo compression before upload (max 1MB)
